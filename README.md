...
> [!IMPORTANT]
> ### Q1: Công thức nhiệt độ Eq.(9)
> Công thức `Tikm = To - 0.5 × (doi / (v × 60))` — đơn vị có vẻ cho ra suy giảm rất nhỏ:
> - Ví dụ: doi=25km, v=40km/h → `0.5 × 25/(40×60) = 0.0052°C`
> - Với To=160°C → Tikm = 159.99°C → gần như không đổi
>
> Bạn có thể xác nhận:
> - Công thức này có đúng chính xác theo tài liệu gốc không?
> - Hay đơn vị thời gian nên là giờ thay vì nhân 60? (nếu `0.5 × doi/v` → `0.5 × 25/40 = 0.3125°C` vẫn rất nhỏ)
> - Hay hệ số 0.5 nên là giá trị lớn hơn (ví dụ 50)?

> [!IMPORTANT]
> ### Q2: Dữ liệu đầu vào cụ thể
> Bạn đã có bộ dữ liệu cụ thể chưa (N, T, doi[], Di[], Q, f, coi[], v, α, Δtdo, To)?
> Hay cần tạo dữ liệu mẫu?

> [!IMPORTANT]
> ### Q3: Ngôn ngữ triển khai
> Giữ Java (khớp codebase) hay chuyển Python?

> [!IMPORTANT]
> ### Q4: Phạm vi so sánh
> Chỉ 3 thuật toán có sẵn (GWO, DA, DA_GWO) hay cần thêm GA, PSO?

## Verification Plan

### Automated Tests
- Tính tay TC cho bộ dữ liệu nhỏ (2 xe, 2 site) → so với `HMAFitness.func()`
- Kiểm tra `repairDemand()`: sau repair, `Σ xikm = ⌈Di/Q⌉` cho mọi i
- Kiểm tra `repairSequence()`: sau repair, `txp[k][m+1] ≥ returnTime[k][m]`
- Chạy DA_GWO_HMA 30 lần → tính Best/Avg/Std

### Manual Verification
- Kiểm tra convergence curve giảm dần
- Xem phương án tối ưu có hợp lý thực tế
- So sánh kết quả 3 thuật toán + kiểm định Mann-Whitney
